...
(defn make-segments [wires]
  (loop [segments []
    ...
    (let [g (re-matches #"([A-Z])([0-9]+)" inst)
          ...
          news (conj segments {:p1 from :p2 np :dir dir :adist adist})
          ]
      (if (seq remain)
        (recur news np (first remain) (rest remain) (+ adist dist))
        news))))
...
(defn find-int [ud lr]
  (let [
        ...
        [lx2 ly2] (:p2 lr)
        rv [ux1 ly1]]
    (if (and (or (> uy1 ly1 uy2) (< uy1 ly1 uy2))
             (or (> lx1 ux1 lx2) (< lx1 ux1 lx2)))
      [rv ud lr]
      nil)))
...
(defn find-intersections [wire1 wire2]
  (let [seg1 (make-segments wire1)
        seg2 (make-segments wire2)
        seg1ud (filter #(or (= (:dir %) "U") (= (:dir %) "D")) seg1)
        seg2lr (filter #(or (= (:dir %) "R") (= (:dir %) "L")) seg2)
        seg2ud (filter #(or (= (:dir %) "U") (= (:dir %) "D")) seg2)
        seg1lr (filter #(or (= (:dir %) "R") (= (:dir %) "L")) seg1)
        ints1 (for [w1 seg1ud
                    w2 seg2lr]
                  (find-int w1 w2))
        ints2 (for [w1 seg2ud
    ...
    (filter (comp not nil?) (into ints1 ints2))
    ))


(defn calc-travel [node]
  (let [
        base (+ (:adist (nth node 1)) (:adist (nth node 2)))
        ytravel (Math/abs (- (nth (:p1 (nth node 1)) 1) (nth (first node) 1)))
        xtravel (Math/abs (- (nth (:p1 (nth node 2)) 0) (nth (first node) 0)))
        ]
    (into [(+ base ytravel xtravel) base ytravel xtravel] node)
    ))
